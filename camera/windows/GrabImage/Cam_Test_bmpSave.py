...
def re_bmp():
    ...
    merged = cv.merge([b, g, r]) #合并R、G、B分量 默认顺序为 B、G、R
...
if __name__ == "__main__":
    ...
    if ret == 0:
        ...
        nRGBSize = (stDeviceList.nWidth * stDeviceList.nHeight * 3)+2048
            # stSaveParam = MV_CC_PIXEL_CONVERT_PARAM()
        ...
        stSaveParam.nImageLen = stDeviceList.nFrameLen
        stSaveParam.nJpgQuality = 70

            # stSaveParam.enDstPixelType = PixelType_Gvsp_RGB8_Packed
        stSaveParam.enImageType = MV_Image_Bmp
        stSaveParam.pImageBuffer = (c_ubyte * nRGBSize)()
        stSaveParam.nBufferSize = nRGBSize
        # ret = cam.MV_CC_ConvertPixelType(stSaveParam)
        ret = cam.MV_CC_SaveImageEx2(stSaveParam)
        if ret != 0:
            print("convert pixel fail aaaaa! ret[0x%x]" % ret)
            del data_buf
            sys.exit()

        file_path = "AfterConvert_RGB2.bmp"
        ...
        cdll.msvcrt.memcpy(byref(img_buff), stSaveParam.pImageBuffer, stSaveParam.nImageLen)  # 获取图像数据
        # 显示图像
        a = np.array(data_buf)
        print(a)
       # cv.resize(img, (400,48))
        abbbbb = cv.imread("AfterConvert_RGB2.jpg")
        print(abbbbb)

        cv.imshow("img",img_buff)
        np.savetxt("a.txt",a)
        file_open.write(img_buff)
